...
def get_sentiment_color(text):
    ...
    # Normalize to 0.0 - 1.0 range for the colormap
...
def get_political_color(url):
    ...
    # HACKATHON SHORTCUT: Manual mapping for common sites
    ...
    return "#95a5a6" # GREY (Neutral/Unknown)

def build_network_graph(articles, sim_matrix, threshold=0.4, color_mode="Cluster"):
    """
    color_mode options: "Cluster", "Sentiment", "Politics"
    """
    ...
    if len(G.edges) > 0:
        ...
        # If we assign groups in 'Sentiment' mode, PyVis ignores our custom colors.
